$(document).ready(function() {
  ...
  $('#login-input').keypress(function(e) {
    if(e.which === 13 && $(this).val() != '') {
      ...
      setTimeout(function(){
        $('span').remove();
        $('#terminal').append('<br><div>Welcome to Nigga Link [version 2.0.0].<br>(c) Copyright 1488 GDCorp Corporation. All rights and wrongs reserved.<br><br>' + player_name + ' authenticated.<br>Compiler ready for use.<br>Use the help command for a list of commands.<br><br><span id="initial-root">$</span>&nbsp;&nbsp;&nbsp;<span id="blinking">_</span></div>');
      }, 6500);
          ...
          if(e.which === 13 && $(this).val() != '' && able === 1) {
            ...
            switch( $(this).val() ) {
              case 'help':
                ...
                help = 1;
                break;
              case 'connect nearest.fent.reactor':
                if (connect === 0) {
                  ...
                  $('#terminal').append('<div>$&nbsp;&nbsp;&nbsp;' + $(this).val() + '<br>You are already connected to nearest.fent.reactor<span id="blinking">_</span></div>');
                }
                break;
              case 'list':
                $('#terminal').append('<div>$&nbsp;&nbsp;&nbsp;' + $(this).val() + '<br>List of currently available applications:<br>Fentanyl Quantum Rifle (type: cyberbioweapon)<br>Propylon data (type: server)<span id="blinking">_</span>');
                list = 1;
                break;
              case 'send Propylon':
                if (connect === 1) {
                  $('#terminal').append('<div>$&nbsp;&nbsp;&nbsp;' + $(this).val() + '<br>Sending Propylon...<br>Propylon sent.<span id="blinking">_</span></div>');
                  trojan = 1;
                } else {
                  $('#terminal').append('<div>$&nbsp;&nbsp;&nbsp;' + $(this).val() + '<br>You are not connected to any DNS name.<span id="blinking">_</span></div>');
                }
                break;
              case 'send Fentanyl Quantum Rifle':
                if (connect === 1) {
                  $('#terminal').append('<div>$&nbsp;&nbsp;&nbsp;' + $(this).val() + '<br>Sending Fentanyl Quantum Rifle...<br>Unable to send cyberbioweapons through Nigga Link 2.0.0.<span id="blinking">_</span></div>');
                } else {
                  $('#terminal').append('<div>$&nbsp;&nbsp;&nbsp;' + $(this).val() + '<br>You are not connected to any DNS name.<span id="blinking">_</span></div>');
                }
                break;
